---
title: "BCCWJ-SPR OT 試行実験 (subjrate, lmer)"
...
## ファイル読み込み
```{r reading_file}
d = read.table("07rt-OT-masked.txt",sep="\t",header=TRUE,fileEncoding="UTF-8")
...
```{r model_all}
model_all <- lmer(SPR_reading_time~SPR_sentence_ID+SPR_bunsetsu_ID+SPR_word_length+SPR_trial+WFR_subj_rate+(1|SPR_subj_ID_factor)+(1|BCCWJ_Sample_ID),data=d,REML=FALSE)
summary(model_all)
...
model_trim = lmer(SPR_reading_time~SPR_sentence_ID+SPR_bunsetsu_ID+SPR_word_length+SPR_trial+WFR_subj_rate+(1|SPR_subj_ID_factor)+(1|BCCWJ_Sample_ID),data=d_trim,REML=FALSE)
summary(model_trim)
...
```{r log_model_all}
log_model_all = lmer(SPR_log_reading_time~SPR_sentence_ID+SPR_bunsetsu_ID+SPR_word_length+SPR_trial+WFR_subj_rate+(1|SPR_subj_ID_factor)+(1|BCCWJ_Sample_ID),data=d,REML=FALSE)
...
log_model_trim = lmer(SPR_log_reading_time~SPR_sentence_ID+SPR_bunsetsu_ID+SPR_word_length+SPR_trial+WFR_subj_rate+(1|SPR_subj_ID_factor)+(1|BCCWJ_Sample_ID),data=d_trim_log,REML=FALSE)
